...
- **Reader bar** — the font/line-spacing controls are always visible, fixed flush at the very bottom of the screen (above the gesture bar via `env(safe-area-inset-bottom)`). The song's bottom padding is **measured at runtime from the bar's actual rendered height** (`max(120, barHeight + 48)`px), so lyrics can never scroll under the bar even with large system font scaling. When a song's lyrics fit on one screen the page scroll is **locked** (nothing to scroll); increasing the font size or line spacing past the fold unlocks scrolling again.
